...
from seq2seq_ver2_tf_idf.tf_idf_trainer import Trainer
from seq2seq_ver2_tf_idf.tf_idf_inputlayer import TF_IDF_InputLayer
from seq2seq_ver2_tf_idf.tf_idf_seq2seq import Seq2seq
...
def tf_cros_valu(idx, wordvec_size, hidden_size, lr, max_epoch, create_fi, date, seed, learn=True):
    ...
    #ファイルへのパラメータ書き込み
    fn = open('../eval/tf-idf/'+date+'.txt', "a")
    para = 'ハイパーパラメータの値\n\
...
max_grad = '+str(max_grad)+'\n'
    ...
    for (x, t, e, s) in zip(train, test, em, summary):
        ...
        if learn:
            ...
            if number == len(test):
                create_fi.loss_plot(len(test), 'TF-IDF', wordvec_size)
